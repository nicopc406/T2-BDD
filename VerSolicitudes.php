...
<div class="container">
    <div class="table-container">
        ...
        <?php if ($tipo_solicitud === 'Funcionalidad'): ?>
            <?php
            $sql = "SELECT id_solicitud, titulo, estado, topico FROM VistaSolicitudesDetalladas WHERE tipo = 'Funcionalidad' AND rut_usuario = ? ORDER BY fecha DESC";
        ...
        <?php elseif ($tipo_solicitud === 'Error'): ?>
            <?php
            $sql = "SELECT id_solicitud, titulo, estado, topico FROM VistaSolicitudesDetalladas WHERE tipo = 'Error' AND rut_usuario = ? ORDER BY fecha DESC";
            $stmt = $conexion->prepare($sql);
            $stmt->bind_param('s', $rut_usuario_actual);
            $stmt->execute();
            $resultado = $stmt->get_result();
